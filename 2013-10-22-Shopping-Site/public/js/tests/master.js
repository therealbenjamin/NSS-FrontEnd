...
test('Add Order', function(){
  ...
  ok(db.orders[0] instanceof Order, 'should be 1 order instance');
  ...
  equal($('#orders tbody .order-time').text().split(' '), 5, 'date time should be formatted');
  equal($('#orders tbody .order-customer').text(), 'Sally', 'should have customer name');
  equal($('#orders tbody .order-total').text(), '$945.00', 'should have customer name');
  equal($('#orders tbody .order-shipping').text(), '$5.25', 'should have customer name');
  equal($('#orders tbody .order-grand').text(), '$950.25', 'should have customer name');
  equal($('#orders tbody .order-products-list li'), 4, 'should have 4 items in order');
});
